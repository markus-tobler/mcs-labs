...
| Expense Claims Agent with Workflow Approvals (Human-in-the-Loop) | [human-in-the-loop-modern](./labs/human-in-the-loop-modern) | Build an expense claims solution with an agent powered by the GitHub Copilot harness. The agent stores receipts in OneDrive and calls a workflow that evaluates each claim against policy with an inline agent, routes higher-value claims to a human reviewer, and emails the requestor the final outcome. |
